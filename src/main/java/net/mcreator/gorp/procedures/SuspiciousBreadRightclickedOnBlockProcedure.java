...
public class SuspiciousBreadRightclickedOnBlockProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.level().clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(10)), ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, entity)).getDirection()) == Direction.DOWN) {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = GorpModEntities.HOSTILE_BREAD.get().spawn(_level, BlockPos.containing(x, y - 2, z), MobSpawnType.MOB_SUMMONED);
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = GorpModEntities.HOSTILE_BREAD.get().spawn(_level, BlockPos.containing(x, y + 1.05, z), MobSpawnType.MOB_SUMMONED);
				if (entityToSpawn != null) {
				}
			}
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = GorpModEntities.HOSTILE_BREAD.get().spawn(_level, BlockPos.containing(x, y, z - 1.8), MobSpawnType.MOB_SUMMONED);
				if (entityToSpawn != null) {
				}
			}
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = GorpModEntities.HOSTILE_BREAD.get().spawn(_level, BlockPos.containing(x - 1.8, y, z), MobSpawnType.MOB_SUMMONED);
				if (entityToSpawn != null) {
				}
			}
		...
		if (!(entity instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(GorpModItems.SUSPICIOUS_BREAD.get());
		...
		if (!world.isClientSide()) {
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("gorp:toast_crunch")), SoundSource.NEUTRAL, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("gorp:toast_crunch")), SoundSource.NEUTRAL, 1, 1, false);
				}
			}
		}
	}
